...
export class ReportsService {
  ...
  async getSummary() {
    ...
    const active = await this.prisma.enrollment.count({ where: { status: 'active' } });
    const completed = await this.prisma.enrollment.count({ where: { status: 'completed' } });
    const dropped = await this.prisma.enrollment.count({ where: { status: 'dropped' } });

    const retention = totalEnrollments ? ((active + completed) / totalEnrollments) * 100 : 0;

    const totalPayments = await this.prisma.payment.count();
    const paidPayments = await this.prisma.payment.count({ where: { status: 'paid' } });
    const pendingPayments = await this.prisma.payment.count({ where: { status: 'pending' } });
    const latePayments = await this.prisma.payment.count({ where: { status: 'late' } });
    ...
    const paymentsStats = {
      ...
      pendingPercent: totalPayments ? (pendingPayments / totalPayments) * 100 : 0,
  ...
  async getGroupReports(groupId: number) {
    ...
    const enrollments = await this.prisma.enrollment.findMany({ where: { groupId } });
    ...
    return {
      ...
      groupName: group.name,
      courseName: group.course.name,
      enrollments: { total, active, completed, dropped, retention: Math.round(retention * 10) / 10 },
      avgProgress: Math.round(avgProgress * 10) / 10,
      attendance: {
        present: presentCount, absent: absentCount, late: lateCount,
        presentPercent: totalAttendance ? (presentCount / totalAttendance) * 100 : 0,
        absentPercent: totalAttendance ? (absentCount / totalAttendance) * 100 : 0,
        latePercent: totalAttendance ? (lateCount / totalAttendance) * 100 : 0,
      },
      payments: {
        total: totalPayments, paid, pending, late,
  ...
  async getGroupRetention(groupId: number) {
    ...
    const active = await this.prisma.enrollment.count({ where: { groupId, status: 'active' } });
    const completed = await this.prisma.enrollment.count({ where: { groupId, status: 'completed' } });
    const dropped = await this.prisma.enrollment.count({ where: { groupId, status: 'dropped' } });
    const retention = total ? ((active + completed) / total) * 100 : 0;
    return { groupId, total, active, completed, dropped, retention: Math.round(retention * 10) / 10 };
  }
  ...
  async getGlobalRetention() {
    ...
    const dropped = await this.prisma.enrollment.count({ where: { status: 'dropped' } });
    const retention = total ? ((active + completed) / total) * 100 : 0;
    return { total, active, completed, dropped, retention: Math.round(retention * 10) / 10 };
  }

  async getRetentionByCourse(courseId: number) {
    const groups = await this.prisma.group.findMany({ where: { courseId }, include: { enrollments: true } });
    let total = 0, retained = 0;
    for (const group of groups) {
      total += group.enrollments.length;
      retained += group.enrollments.filter((e) => e.status === 'active' || e.status === 'completed').length;
    }
